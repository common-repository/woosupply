...
class SupportPage extends \LWS\Adminpanel\Internal\Credits\Page
{
	...
	private function getDebugInfo()
	{
		...
		$data = array(
			...
			sprintf("License: %s", \get_site_option('lws-license-key-' . $this->getSlug())),
			sprintf("Website: %s", (defined('WP_SITEURL') && WP_SITEURL) ? WP_SITEURL : \get_site_option('siteurl')),
	...
	private function getMailValues()
	{
		$values = array(
			...
			'lws_adm_support_email'     => isset($_POST['lws_adm_support_user_name']) ? \trim(\sanitize_email($_POST['lws_adm_support_email'])) : false,
			'lws_adm_support_subject'   => isset($_POST['lws_adm_support_user_name']) ? \trim(\sanitize_text_field($_POST['lws_adm_support_subject'])) : false,
			'lws_adm_support_request'   => isset($_POST['lws_adm_support_user_name']) ? \trim(\wp_unslash($_POST['lws_adm_support_request'])) : false,
		);
	...
	private function restoreLastValues()
	{
		...
		$values = array(
			...
			'lws_adm_support_email'     => $user->user_email,
			...
			'fields' => array(
				...
				'email' => array(
					...
					'extra' => array(
						...
						'size'	=> '40',
						'gizmo' => true,
						'help'  => __("The email we will response to. Please make sure this is a valid email that you view frequently.", 'lws-adminpanel'),
					)
				),
				'subject' => array(
